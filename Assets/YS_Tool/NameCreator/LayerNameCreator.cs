...
namespace Other_Tool.Constant {
    ...
    public static class LayerNameCreator {
        ...
        private const string PATH = "Assets/Constant/LayerName.cs"; // ファイルパス
        ...
        public static void CreateScript() {
            ...
            foreach (var n in InternalEditorUtility.layers.Select(c =>
                new {var = RemoveInvalidChars(c), val = LayerMask.NameToLayer(c)})) {
                builder.Append("\t").AppendFormat(@"public const int {0} = {1};", n.var, n.val).AppendLine();
            }

            foreach (var n in InternalEditorUtility.layers.Select(c =>
                new {var = RemoveInvalidChars(c), val = 1 << LayerMask.NameToLayer(c)})) {
